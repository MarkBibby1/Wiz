...
locals {
  wiz_da_worker_principal_id = var.use_worker_managed_identity ? azurerm_user_assigned_identity.wiz_da_worker_identity[0].principal_id : azuread_service_principal.wiz_da_worker_sp[0].id
}
...
resource "azurerm_key_vault" "wiz_outpost_keyvault" {
  ...
  access_policy {
    tenant_id          = var.azure_tenant_id
    object_id          = azuread_service_principal.wiz_da_orchestrator_sp.id
