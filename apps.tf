...
resource "azuread_application_password" "wiz_da_orchestrator_pass" {
  ...
  end_date_relative = var.key_expire_end_date_relative # 24h * 365 days * 10 = 10 years
}
...
# Default is "false"
locals {
  # If multi_tenancy_enabled is true -> "AzureADMultipleOrgs", else "AzureADandPersonalMicrosoftAccount"
  sign_in_aud = (var.multi_tenancy_enabled ? "AzureADMultipleOrgs" : "AzureADandPersonalMicrosoftAccount")
}
...
resource "azuread_application_password" "wiz_da_worker_pass" {
  ...
  end_date_relative = var.key_expire_end_date_relative # 24h * 365 days * 10 years
}
